...
```
    ...
    ## Error in eval(expr, envir, enclos): object 'range_results' not found
...
```
    ...
    ## Error in eval(expr, envir, enclos): object 'larger_value' not found
...
diff\_range(gapminder*l**i**f**e**E**x**p*)*d**i**f**f*<sub>*r*</sub>*a**n**g**e*(1 : 10)*d**i**f**f*<sub>*r*</sub>*a**n**g**e*(5 : 100)*d**i**f**f*<sub>*r*</sub>*a**n**g**e*(*g**a**p**m**i**n**d**e**r*pop)
diff\_range(gapminder$gdpPercap)
...
quantile\_range &lt;- function(x) {

rng = quantile(x, probs = c(0,1)) return(diff\_range(rng))
...
quantile\_range(1:10) quantile\_range(5:100)
quantile\_range(gapminder*p**o**p*)*q**u**a**n**t**i**l**e*<sub>*r*</sub>*a**n**g**e*(*g**a**p**m**i**n**d**e**r*gdpPercap)
...
quantile\_range(gapminder$country)
...
rng = quantile(x, probs = c(0,1))
return(diff_range(rng))
...
``` r
quantile_IQR <- function(x) {

rng = quantile(x, probs = c(0.25,0.75))
return(diff_range(rng))
}
```
...
``` r
quantile_range2 <- function(x,probs){
  if probs == ignore.caps("range"){
    return(diff_range(quantile_range(gapminder$lifeExp, probs = c(0.25, 0.75))))
  }else if probs == ignore.caps("iqr") {
    return(diff_range(quantile_range(gapminder$lifeExp, probs = c(0, 1))))
  }
}
...
```
    ...
    ## 1: quantile_range2 <- function(x,probs){
    ## 2:   if probs
    ##         ^
...
```
    ...
    ## Error in z[3] <- NA: object 'z' not found

``` r
quantile(gapminder$lifeExp)
```

    ## Error in quantile(gapminder$lifeExp): object 'gapminder' not found

``` r
quantile(z)
```

    ## Error in quantile(z): object 'z' not found

``` r
quantile(z, na.rm = TRUE)
```

    ## Error in quantile(z, na.rm = TRUE): object 'z' not found
